...
static int ICACHE_FLASH_ATTR
calc_img_crc(uint16 fw_bin_sec, unsigned int sumlength, unsigned int *img_crc)
{
//	int fd;
//	int ret;
	int nFlashAddr = 0;
...
//	uint32 sec_last = sumlength % CRC_BLOCK_SIZE;
	for (nFlashAddr = fw_bin_sec * SPI_FLASH_SEC_SIZE;
			nFlashAddr < fw_bin_sec * SPI_FLASH_SEC_SIZE + sumlength; nFlashAddr++) {
		if ( 0 != (error = spi_flash_read(nFlashAddr, &unbuf, 1))){
				os_printf("spi_flash_read error %d\n",error);
				return (-1);
		}
		ucbuf = unbuf & 0xFF;
//		if (nFlashAddr < (fw_bin_sec * SPI_FLASH_SEC_SIZE + 8)){
//			printf("0x%02X ", ucbuf);
//		}
		crc = xcrc32(&ucbuf, 1, crc);
	}
...
upgrade_crc_check(uint16 fw_bin_sec, unsigned int sumlength)
{
	...
	ret = calc_img_crc(fw_bin_sec, sumlength - 4, &img_crc);
	if (ret < 0) {
		return false;
	}
	os_printf("CRC calculated from programmed flash is: 0x%08X\n", img_crc);
	spi_flash_read(fw_bin_sec * SPI_FLASH_SEC_SIZE + sumlength - 4, &flash_crc, 4);
